...
const CombinedEfficiencyCard = () => {
  ...
  const combinedData = useMemo(() => {
    ...
    // Energy efficiency data - appliances and systems
    const applianceData = [
      {
        name: "Space heater",
        consumption: 300,
  ...
  return (
    <Accordion defaultExpanded sx={{ height: "100%" }}>
      <AccordionSummary
        expandIcon={<ExpandMore />}
        aria-controls="space-efficiency-content"
        id="space-efficiency-header"
        sx={{
            ...
            <Box sx={{ display: "flex", gap: 3, mb: 2 }}>
              ...
              <Box sx={{ flex: 1 }}>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                  {combinedData.occupancyData.slice(0, 6).map((room, index) => (
                    <Box
                      key={index}
                      sx={{
                        display: "flex",

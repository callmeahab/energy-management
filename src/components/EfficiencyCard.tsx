...
interface CombinedEfficiencyCardProps {
  drawerOpen?: boolean;
}

const EfficiencyCard = ({
  drawerOpen = false,
}: CombinedEfficiencyCardProps) => {
  const theme = useTheme();
  const isSmallScreen = useMediaQuery(theme.breakpoints.down("md"));
  const shouldStack = drawerOpen && isSmallScreen;
      ...
      <CardContent sx={{ p: 0 }}>
        ...
        <Collapse in={expanded} timeout="auto" unmountOnExit>
          {buildingsLoading ? (
            ...
            <Box>
              {/* Three column layout (stacks on small screens when drawer is open) */}
              <Box
                sx={{
                  display: "flex",
                  gap: 3,
